---
content_title: ksigd Usage
---

[[info | Recommended Usage]]
| For most users, the easiest way to use `ksigd` is to have `clsig` launch it automatically. Wallet files will be created in the default directory (`~/sig-wallet`).

## Launching ksigd manually

`ksigd` can be launched manually from the terminal by running:

```sh
ksigd
```

By default, `ksigd` creates the folder `~/sig-wallet` and populates it with a basic `config.ini` file.  The location of the config file can be specified on the command line using the `--config-dir` argument.  The configuration file contains the HTTP server endpoint for incoming HTTP connections and other parameters for cross-origin resource sharing.
...
By default, `ksigd` is set to lock your wallet after 15 minutes of inactivity. This is configurable in the `config.ini` by setting the timeout seconds in `unlock-timeout`. Setting it to 0 will cause `ksigd` to always lock your wallet.

## Stopping ksigd

The most effective way to stop `ksigd` is to find the ksigd process and send a SIGTERM signal to it.

## Other options

For a list of all commands known to `ksigd`, simply run it with no arguments:
...
Config Options for eosio::http_plugin:
  --unix-socket-path arg (=ksigd.sock)  The filename (relative to data-dir) to
