---
content_title: nodsig Plugins
---

## Overview

Plugins extend the core functionality implemented in `nodsig`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `nodsig`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
[[info | nodsig is modular]]
| Plugins add incremental functionality to `nodsig`. Unlike runtime plugins, `nodsig` plugins are built at compile-time.
