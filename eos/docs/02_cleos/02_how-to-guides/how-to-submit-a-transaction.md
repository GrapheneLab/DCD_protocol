...
[[info]]
| Be aware that if a clear text `data` field is used, clsig need to fetch copies of required ABIs using `nodsig` API. That operation has a performance overhead on `nodsig`
...
```sh
clsig push transaction TRX_FILE.json
```

* Submit a transaction from a JSON:

```sh
clsig push transaction JSON
```
