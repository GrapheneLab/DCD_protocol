...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if SIG_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "SIG_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
...
void chain_plugin::handle_guard_exception(const chain::guard_exception& e) {
   ...
   elog("database chain::guard_exception, quiting..."); // log string searched for in: tests/nodsig_under_min_avail_ram.py
   // quit the app
   app().quit();
}
...
void chain_plugin::handle_db_exhaustion() {
   ...
   //return 1 -- it's what programs/nodsig/main.cpp considers "BAD_ALLOC"
   std::_Exit(1);
}

void chain_plugin::handle_bad_alloc() {
   elog("std::bad_alloc - memory exhausted");
   //return -2 -- it's what programs/nodsig/main.cpp reports for std::exception
   std::_Exit(-2);
}
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(sig.token);
...
read_only::get_action_fee_result read_only::get_action_fee( const get_action_fee_params& params )const {
   auto res = db.get_txfee_manager().get_action_fee(db, params.account, params.action);
...
read_only::get_fee_rate_result read_only::get_fee_rate(const get_fee_rate_params& params)const {
   auto res = db.get_txfee_manager().get_fee_rate(db);
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   const auto& d = db.db();
   const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( N(sig), N(sig), N(rammarket) ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
