...
function usage() {
  ...
  -c DIR      Directory where SIG.CDT is installed. (Default: /usr/local/sig.cdt)
  -t          Build unit tests.
...
if [[ ${BUILD_TESTS} == true ]]; then
   # Prompt user for location of sig.
   eosio-directory-prompt
fi

# Prompt user for location of sig.cdt.
cdt-directory-prompt

# Include CDT_INSTALL_DIR in CMAKE_FRAMEWORK_PATH
echo "Using SIG.CDT installation at: $CDT_INSTALL_DIR"
