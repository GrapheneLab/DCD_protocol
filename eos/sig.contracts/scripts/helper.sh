...
# Prompts or default behavior for choosing SIG.CDT directory.
function cdt-directory-prompt() {
#  if [[ -z $CDT_DIR_PROMPT ]]; then
#    echo 'No SIG.CDT location was specified.'
#    while true; do
#      if [[ $NONINTERACTIVE != true ]]; then
#        printf "Is SIG.CDT installed in the default location? /usr/local/sig.cdt (y/n)" && read -p " " PROCEED
#      fi
#      echo ""
#      case $PROCEED in
#        "" )
#          echo "Is SIG.CDT installed in the default location?";;
#        0 | true | [Yy]* )
#          break;;
#        1 | false | [Nn]* )
#          printf "Enter the installation location of SIG.CDT:" && read -e -p " " CDT_DIR_PROMPT;
...
#    done
# fi
  export CDT_INSTALL_DIR="/home/olga/opt/sig.cdt"
}
