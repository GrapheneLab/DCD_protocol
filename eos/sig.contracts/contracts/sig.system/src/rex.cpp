#include <sig.system/sig.system.hpp>
#include <sig.token/sig.token.hpp>
#include <sig.system/rex.results.hpp>
...
namespace eosiosystem {
   ...
   void system_contract::runrex( uint16_t max )
   {
      ...
      /// transfer from sig.names to sig.rex
   ...
   void system_contract::channel_to_rex( const name& from, const asset& amount )
   {
      ...
      if ( rex_available() ) {
         ...
         transfer_act.send( from, rex_account, amount,
                            std::string("transfer from ") + from.to_string() + " to sig.rex" );
      }
#endif
   }
