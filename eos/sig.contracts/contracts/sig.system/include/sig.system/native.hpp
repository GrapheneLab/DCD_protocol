...
namespace eosiosystem {
   ...
   struct block_header {
      ...
      std::optional<eosio::rate_schedule>       new_rate;
    ...
    */
   struct [[eosio::table("abihash"), eosio::contract("sig.system")]] abi_hash {
      name              owner;
      checksum256       hash;
    ...
    */
   class [[eosio::contract("sig.system")]] native : public eosio::contract {
      public:
          ...
          * validation otherwise then active is the default, with the exception of `sig.any`.
          * `sig.any` is an implicit permission which exists on every account; you can link actions to `sig.any`
