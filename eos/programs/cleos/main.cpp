/**
  ...
  @section intro Introduction to clsig

  `clsig` is a command line tool that interfaces with the REST api exposed by @ref nodsig. In order to use `clsig` you will need to
  have a local copy of `nodsig` running and configured to load the 'eosio::chain_api_plugin'.

   clsig contains documentation for all of its commands. For a list of all commands known to clsig, simply run it with no arguments:
```
$ ./clsig
Command Line Interface to EOSIO Client
Usage: programs/clsig/clsig [OPTIONS] SUBCOMMAND
...
//copy pasta from ksigd's main.cpp
...
string default_wallet_url = "unix://" + (determine_home_directory() / "sig-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
void ensure_keosd_running(CLI::App* app) {
    if (no_auto_keosd)
        return;
    // get, version, net, convert do not require ksigd
    ...
    // This extra check is necessary when running clsig like this: ./clsig ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if clsig and ksigd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("ksigd").append(key_store_executable_name);
    }
...
struct set_fee_rate_producer_subcommand {
   ...
   set_fee_rate_producer_subcommand(CLI::App* actionRoot) {
      ...
      set_fee_rate_producer->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ("producer", producer_str)
                  ("fee_rate", producer_fee_rate_str);

         auto accountPermissions = get_account_permissions(tx_permission, {name(producer_str), config::active_name});
         send_actions({create_action(accountPermissions, config::system_account_name, N(setrateprod), act_payload)});
      });
   }
};
...
struct confirm_producer_subcommand {
   ...
   confirm_producer_subcommand(CLI::App* actionRoot) {
      ...
      confirm_producer->set_callback([this] {
         ...
         send_actions({create_action({permission_level{ permission_account, config::active_name }}, config::system_account_name, N(confirmprod), act_payload )});
      });
   }
};
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clsig can still work with old buggy nodsig versions
                               // Change to newname.value when clsig no longer needs to support nodsig versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         ...
         // Condition in if statement below can simply be res.rows.empty() when clsig no longer needs to support nodsig versions older than 1.5.0
   ...
   if (!json_format) {
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of nodsig responding to the request is different than core symbol built into clsig
...
int main( int argc, char** argv ) {
   ...
   getCode->set_callback([&] {
      ...
      try {
         ...
         //see if this is an old nodsig that doesn't support get_raw_code_and_abi
   ...
   auto getFeeRateInformation = get->add_subcommand("fee_rate_info", localized("Get fee rate information"), false);
   getFeeRateInformation->set_callback([&] {
...
//                                ->check(CLI::ExistingFile);
   ...
   stopKeosd->set_callback([] {
      ...
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success ksigd responds with empty object
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(sig.msig), N(propose), variant_to_bin( N(sig.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so clsig can still work with old buggy nodsig versions
                                 // Change to name(proposal_name).value when clsig no longer needs to support nodsig versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when clsig no longer needs to support nodsig versions older than 1.5.0
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(sig.msig), name(action), variant_to_bin( N(sig.msig), name(action), args ) }});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(sig.msig), N(invalidate), variant_to_bin( N(sig.msig), N(invalidate), args ) }});
   });
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, N(sig.msig), N(cancel), variant_to_bin( N(sig.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(sig.msig), N(exec), variant_to_bin( N(sig.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send sig.system contract action to the blockchain."), false);
   ...
   auto confirmProducer = confirm_producer_subcommand(system);
