...
def startWallet():
    ...
    background(args.ksigd + ' --unlock-timeout %d --http-server-address 127.0.0.1:6666 --wallet-dir %s' % (unlockTimeout, os.path.abspath(args.wallet_dir)))
    sleep(.4)
    run(args.clsig + 'wallet create --to-console')

def importKeys():
    run(args.clsig + 'wallet import --private-key ' + args.private_key)
    keys = {}
    for a in accounts:
        key = a['pvt']
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.clsig + 'wallet import --private-key ' + key)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.clsig + 'create account eosio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.clsig + 'system newaccount --transfer eosio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.clsig + 'transfer eosio %s "%s"' % (a['name'], intToCurrency(unstaked)))

def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.clsig + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.clsig + 'system listproducers')
...
def vote(b, e):
    for i in range(b, e):
        ...
        retry(args.clsig + 'system voteproducer prods ' + voter + ' ' + prods)

def claimRewards():
    table = getJsonOutput(args.clsig + 'get table eosio eosio producers -l 100')
    times = []
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.clsig + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
    print('Elapsed time for claimrewards:', times)

def proxyVotes(b, e):
    vote(firstProducer, firstProducer + 1)
    proxy = accounts[firstProducer]['name']
    retry(args.clsig + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.clsig + 'system voteproducer proxy ' + voter + ' ' + proxy)

def updateAuth(account, permission, parent, controller):
    run(args.clsig + 'push action eosio updateauth' + jsonArg({
        'account': account,
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.clsig + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.clsig + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    run(args.clsig + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'eosio setcode' + jsonArg(setcode) + ' -p ' + proposer)

def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.clsig + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.clsig + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.clsig + 'push action eosio buyrambytes' + jsonArg(['eosio', accounts[0]['name'], 200000]) + '-p eosio')
    sleep(1)
...
def produceNewAccounts():
    with open('newusers', 'w') as f:
        for i in range(120_000, 200_000):
            x = getOutput(args.clsig + 'create key --to-console')
...
def stepInstallSystemContracts():
    run(args.clsig + 'set contract sig.token ' + args.contracts_dir + '/sig.token/')
    run(args.clsig + 'set contract eosio.msig ' + args.contracts_dir + '/eosio.msig/')
def stepCreateTokens():
    run(args.clsig + 'push action sig.token create \'["eosio", "10000000000.0000 %s"]\' -p sig.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.clsig + 'push action sig.token issue \'["eosio", "%s", "memo"]\' -p eosio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    ...
    retry(args.clsig + 'set contract eosio ' + args.old_contracts_dir + '/eosio.system/')
    sleep(3)

    # activate remaining features
    # GET_SENDER
    retry(args.clsig + 'push action eosio activate \'["f0af56d2c5a48d60a4a5b5c903edfb7db3a736a94ed589d0b797df33ff9d3e1d"]\' -p eosio')
    # FORWARD_SETCODE
    retry(args.clsig + 'push action eosio activate \'["2652f5f96006294109b3dd0bbde63693f55324af452b799ee137a81a905eed25"]\' -p eosio')
    # ONLY_BILL_FIRST_AUTHORIZER
    retry(args.clsig + 'push action eosio activate \'["8ba52fe7a3956c5cd3a656a3174b931d3bb2abb45578befc59f283ecd816a405"]\' -p eosio')
    # RESTRICT_ACTION_TO_SELF
    retry(args.clsig + 'push action eosio activate \'["ad9e3d8f650687709fd68f4b90b41f7d825a365b02c23a636cef88ac2ac00c43"]\' -p eosio@active')
    # DISALLOW_EMPTY_PRODUCER_SCHEDULE
    retry(args.clsig + 'push action eosio activate \'["68dcaa34c0517d19666e6b33add67351d8c5f69e999ca1e37931bc410a297428"]\' -p eosio@active')
     # FIX_LINKAUTH_RESTRICTION
    retry(args.clsig + 'push action eosio activate \'["e0fb64b1085cc5538970158d05a009c24e276fb94e1a0bf6a528b48fbc4ff526"]\' -p eosio@active')
     # REPLACE_DEFERRED
    retry(args.clsig + 'push action eosio activate \'["ef43112c6543b88db2283a2e077278c315ae2c84719a8b25f25cc88565fbea99"]\' -p eosio@active')
    # NO_DUPLICATE_DEFERRED_ID
    retry(args.clsig + 'push action eosio activate \'["4a90c00d55454dc5b059055ca213579c6ea856967712a56017487886a4d4cc0f"]\' -p eosio@active')
    # ONLY_LINK_TO_EXISTING_PERMISSION
    retry(args.clsig + 'push action eosio activate \'["1a99a59d87e06e09ec5b028a9cbb7749b4a5ad8819004365d02dc4379a8b7241"]\' -p eosio@active')
    # RAM_RESTRICTIONS
    retry(args.clsig + 'push action eosio activate \'["4e7bf348da00a945489b2a681749eb56f5de00b900014e137ddae39f48f69d67"]\' -p eosio@active')
    # WEBAUTHN_KEY
    retry(args.clsig + 'push action eosio activate \'["4fca8bd82bbd181e714e283f83e1b45d95ca5af40fb89ad3977b653c448f78c2"]\' -p eosio@active')
    # WTMSIG_BLOCK_SIGNATURES
    retry(args.clsig + 'push action eosio activate \'["299dcb6af692324b899b39f16d5a530a33062804e41f09dc97e9f156b4476707"]\' -p eosio@active')
    sleep(1)

    run(args.clsig + 'push action eosio setpriv' + jsonArg(['eosio.msig', 1]) + '-p eosio@active')

    # install eosio.system latest version
    retry(args.clsig + 'set contract eosio ' + args.contracts_dir + '/eosio.system/')
    sleep(3)

def stepInitSystemContract():
    run(args.clsig + 'push action eosio init' + jsonArg(['0', '4,' + args.symbol]) + '-p eosio@active')
    sleep(1)
...
commands = [
    ('k', 'kill',               stepKillAll,                True,    "Kill all nodsig and ksigd processes"),
    ('w', 'wallet',             stepStartWallet,            True,    "Start ksigd, create wallet, fill with keys"),
...
parser.add_argument('--clsig', metavar='', help="clsig command", default='../../build/programs/clsig/clsig --wallet-url http://127.0.0.1:6666 ')
parser.add_argument('--nodsig', metavar='', help="Path to nodsig binary", default='../../build/programs/nodsig/nodsig')
parser.add_argument('--ksigd', metavar='', help="Path to ksigd binary", default='../../build/programs/ksigd/ksigd')
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for clsig')
...
args.clsig += '--url http://127.0.0.1:%d ' % args.http_port
