...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(sig.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create sig.msig and sig.token
        create_accounts({N(sig.msig), N(sig.token), N(sig.ram), N(sig.ramfee), N(sig.stake), N(sig.vpay), N(sig.bpay), N(sig.saving) });
        // Set code for the following accounts:
        //  - eosio (code: sig.bios) (already set by tester constructor)
        //  - sig.msig (code: sig.msig)
        //  - sig.token (code: sig.token)
        // set_code_abi(N(sig.msig), contracts::eosio_msig_wasm(), contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        // set_code_abi(N(sig.token), contracts::eosio_token_wasm(), contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        set_code_abi(N(sig.msig),
                     contracts::eosio_msig_wasm(),
                     contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        set_code_abi(N(sig.token),
                     contracts::eosio_token_wasm(),
                     contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        // Set privileged for sig.msig and sig.token
        set_privileged(N(sig.msig));
        set_privileged(N(sig.token));

        // Verify sig.msig and sig.token is privileged
        const auto& eosio_msig_acc = get<account_metadata_object, by_name>(N(sig.msig));
        BOOST_TEST(eosio_msig_acc.is_privileged() == true);
        const auto& eosio_token_acc = get<account_metadata_object, by_name>(N(sig.token));
        BOOST_TEST(eosio_token_acc.is_privileged() == true);


        // Create SYS tokens in sig.token, set its manager as eosio
        ...
        create_currency(N(sig.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to sig.system
        issue(N(sig.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(sig.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
