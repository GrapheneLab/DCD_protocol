...
try {
   ...
   // Creating account with sig. prefix with privileged account
   chain.create_account(name("sig.test1"));

   // Creating account with sig. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("sig.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'sig.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("eosio"), name("sig.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("eosio"), name("sig.any"), name("reqauth") );
