...
getHeadBlockNum()
{
  INFO="$(programs/clsig/clsig get info)"
  verifyErrorCode "clsig get info"
...
# stand up nodsig cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodsig \"--plugin eosio::wallet_api_plugin\" $launcherOpts
programs/eosio-launcher/eosio-launcher --nodsig "--plugin eosio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clsig/clsig --port $port2 get block 1)"
    verifyErrorCode "clsig get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clsig/clsig create key)"
verifyErrorCode "clsig create key"
...
PASSWORD_INITA="$(programs/clsig/clsig wallet create --name inita)"
verifyErrorCode "clsig wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clsig/clsig wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clsig wallet import"
programs/clsig/clsig wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clsig wallet import"
programs/clsig/clsig wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clsig wallet import"
programs/clsig/clsig wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clsig wallet import"
...
ACCOUNT_INFO="$(programs/clsig/clsig create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clsig create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clsig/clsig get account testera)"
verifyErrorCode "clsig get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clsig/clsig transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clsig transfer"
   ...
   TRANS_INFO="$(programs/clsig/clsig --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clsig get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
