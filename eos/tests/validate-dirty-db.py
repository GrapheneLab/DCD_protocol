...
# validate-dirty-db
#
# Test for validating the dirty db flag sticks repeated nodsig restart attempts
#
###############################################################
...
def runnodsigAndGetOutput(myTimeout=3):
    """Startup nodsig, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching nodsig process.")
    cmd="programs/nodsig/nodsig --config-dir etc/eosio/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false"
    Print("cmd: %s" % (cmd))
    proc=subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if debug: Print("nodsig process launched.")

    output={}
    try:
        if debug: Print("Setting nodsig process timeout.")
        outs,errs = proc.communicate(timeout=myTimeout)
        if debug: Print("nodsig process has exited.")
        output["stdout"] = outs.decode("utf-8")
        output["stderr"] = errs.decode("utf-8")
        output["returncode"] = proc.returncode
    except (subprocess.TimeoutExpired) as _:
        Print("ERROR: nodsig is running beyond the defined wait time. Hard killing nodsig instance.")
...
try:
    ...
    Print("Restart nodsig repeatedly to ensure dirty database flag sticks.")
    timeout=6

    for i in range(1,4):
        Print("Attempt %d." % (i))
        ret = runnodsigAndGetOutput(timeout)
        assert(ret)
        assert(isinstance(ret, tuple))
        if not ret[0]:
            errorExit("Failed to startup nodsig successfully on try number %d" % (i))
        assert(ret[1])
