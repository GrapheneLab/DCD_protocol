...
###############################################################
# nodsig_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of nodsig can work with newer versions of nodsig.
#
###############################################################
...
def restartNode(node: Node, nodeId, chainArg=None, addSwapFlags=None, nodsigPath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(nodeId, chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, cachePopen=True, nodsigPath=nodsigPath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extranodsigArgs=" --plugin eosio::producer_api_plugin ",
                          useBiosBootFile=False,
                          specificExtranodsigArgs={
    ...
    # Finally, when we restart the 4th node with the version of nodsig that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodsigPath="programs/nodsig/nodsig")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodsigPath="programs/nodsig/nodsig")
