...
###############################################################
# nodsig_run_remote_test
#
#  Tests remote capability of the nodsig_run_test. Test will setup cluster and pass nodes info to nodsig_run_test. E.g.
#  nodsig_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodsig_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodsig", actualTest))
    Print("cmd: %s\n" % (cmd))
