...
nodsig_LAUNCH_PARAMS="./programs/nodsig/nodsig -d $EOSIO_STUFF_DIR --config-dir $EOSIO_STUFF_DIR \
...
run_nodsig() {
   if (( $VERBOSE == 0 )); then
      $nodsig_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" 2>/dev/null &
   else
      $nodsig_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" &
   fi
}
...
run_expect_failure() {
   ...
   trap "kill $nodsig_PID; wait $nodsig_PID; exit 1" ALRM
   sleep 10
   if wait $nodsig_PID; then exit 1; fi
   kill $TIMER_PID
   trap ALRM
}
