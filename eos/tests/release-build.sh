...
echo ''
echo '    The purpose of this test is to ensure that nodsig was built with compiler'
echo 'optimizations enabled. While there is no way to programmatically determine that'
echo 'given one binary, we do set a debug flag in nodsig when it is built with'
echo 'asserts. This test checks that debug flag. Anyone intending to build and install'
echo 'nodsig from source should perform a "release build" which excludes asserts and'
...
# find nodsig
[[ $(git --version) ]] && cd "$(git rev-parse --show-toplevel)/build/programs/nodsig" || cd "$(dirname "${BASH_SOURCE[0]}")/../programs/nodsig"
if [[ ! -f nodsig ]]; then
    echo 'ERROR: nodsig binary not found!'
    echo ''
    echo 'I looked here...'
    echo "$ ls -la \"$(pwd)/programs/nodsig\""
    ls -la "$(pwd)/programs/nodsig"
...
# run nodsig to generate state files
./nodsig --extract-build-info build-info.json 1>/dev/null 2>/dev/null
