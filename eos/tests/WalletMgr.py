...
class WalletMgr(object):
    __walletLogOutFile="test_ksigd_out.log"
    __walletLogErrFile="test_ksigd_err.log"
    __walletDataDir="test_wallet_0"
    __MaxPort=9999

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(ksigd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodsigPort=8888, nodsigHost="localhost", port=9899, host="localhost"):
        self.walletd=walletd
        self.nodsigPort=nodsigPort
        self.nodsigHost=nodsigHost
        self.port=port
        self.host=host
        self.wallets={}
    ...
    def getArgs(self):
        return " --url http://%s:%d%s %s" % (self.nodsigHost, self.nodsigPort, self.getWalletEndpointArgs(), Utils.MiscEosClientArgs)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch ksigd")
            return False
