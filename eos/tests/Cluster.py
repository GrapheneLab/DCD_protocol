...
class Cluster(object):
    ...
    # walletd [True|False] Is ksigd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
        ...
        """Cluster container.
        walletd [True|False] Is wallet ksigd running. If not load the wallet plugin
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extranodsigArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtranodsigArgs=None, onlySetProds=False,
        ...
        extranodsigArgs: string of arguments to pass through to each nodoes instance (via --nodsig flag on launcher)
        ...
        specificExtranodsigArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodsig flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodsig default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        nodsigArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodsigArgs += " --plugin eosio::wallet_api_plugin"
        if self.enableMongo:
            nodsigArgs += " --plugin eosio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extranodsigArgs is not None:
            assert(isinstance(extranodsigArgs, str))
            nodsigArgs += extranodsigArgs
        if Utils.Debug:
            nodsigArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodsigArgs += " --plugin eosio::producer_api_plugin"

        if nodsigArgs:
            cmdArr.append("--nodsig")
            cmdArr.append(nodsigArgs)

        if specificExtranodsigArgs is not None:
            assert(isinstance(specificExtranodsigArgs, dict))
            for nodeNum,arg in specificExtranodsigArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodsig")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodsig")
                cmdArr.append(path)
        ...
        for name, keys in producerKeys.items():
            ...
            if name != "eosio":
                ...
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from sig.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        eosioTokenAccount.name="sig.token"
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodsig instances. allInstances will kill all nodsig instances running on the system."""
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodsig instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodsig instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
