...
namespace eosio { namespace chain {
   ...
   bool transaction_context::need_fee_action() const {
      if(fee_payer == N(sig) ||
         fee_payer == N(sig.bpay) ||
         fee_payer == N(sig.msig) ||
         fee_payer == N(sig.names) ||
         fee_payer == N(sig.ram) ||
         fee_payer == N(sig.ramfee) ||
         fee_payer == N(sig.saving) ||
         fee_payer == N(sig.stake) ||
         fee_payer == N(sig.token) ||
         fee_payer == N(sig.vpay) ||
         fee_payer == N(sig.rex) ||
         fee_payer == N(sig.feebank) ||
         fee_payer == N(siggateway)
         ) 
         return false;
      else {
         return true;
   ...
   void transaction_context::schedule_fee_action() {
      if(need_fee_action()) {
         const auto fee = control.get_txfee_manager().get_required_fee(control, trx);
