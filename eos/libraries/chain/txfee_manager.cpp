...
namespace eosio { namespace chain {
   ...
   txfee_manager::txfee_manager(){
      init_native_fee(config::system_account_name, N(defaultfee), asset::from_string("1.0000 USD"));
   }
   ...
   void  txfee_manager::get_new_rate_params( const controller& ctl ) {
      contract_cur_rate = ctl.active_rate().rate;
   }

   asset txfee_manager::get_base_rate_asset(const controller& ctl) const {
      return base_rate_asset;
   }

   fee_rate_info txfee_manager::get_fee_rate(const controller& ctl) const {
   ...
   asset txfee_manager::get_core_from_usd(const asset& usd) const {
   ...
   action_fee_info txfee_manager::get_action_fee(const controller& ctl, const account_name& account, const action_name& act ) const {
      ...
      const auto default_fee_in_db = db.find<action_fee_object, by_action_name>(
         boost::make_tuple(config::system_account_name, N(defaultfee)));
      ...
      info.core_fee = get_native_fee(config::system_account_name, N(defaultfee));
      const auto itr = fee_map.find(std::make_pair(config::system_account_name, N(defaultfee)));
   ...
   asset txfee_manager::get_required_fee( const controller& ctl, const account_name& account, const action_name& act ) const {
      ...
      return get_native_fee(config::system_account_name, N(defaultfee));
   }
