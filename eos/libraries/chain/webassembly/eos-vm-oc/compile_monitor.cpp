...
struct compile_monitor_session {
   compile_monitor_session(boost::asio::io_context& context, local::datagram_protocol::socket&& n, wrapped_fd&& c, wrapped_fd& t) :
      _ctx(context),
      _nodsig_instance_socket(std::move(n)),
   ...
   void read_message_from_nodsig() {
      _nodsig_instance_socket.async_wait(local::datagram_protocol::socket::wait_read, [this](auto ec) {
         if(ec) {
            connection_dead_signal();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_nodsig_instance_socket);
         if(!success) {
            connection_dead_signal();
            return;
         }
   ...
   void kick_compile_off(const code_tuple& code_id, wrapped_fd&& wasm_code) {
      ...
      if(write_message_with_fds(_trampoline_socket, trampoline_compile_request, fds_pass_to_trampoline) == false) {
         ...
         write_message_with_fds(_nodsig_instance_socket, reply);
         return;
      }
...
private:
   ...
   local::datagram_protocol::socket _nodsig_instance_socket;
...
struct compile_monitor {
   compile_monitor(boost::asio::io_context& ctx, local::datagram_protocol::socket&& n, wrapped_fd&& t) : _nodsig_socket(std::move(n)), _trampoline_socket(std::move(t)) {
   ...
   void wait_for_new_incomming_session(boost::asio::io_context& ctx) {
      _nodsig_socket.async_wait(boost::asio::local::datagram_protocol::socket::wait_read, [this, &ctx](auto ec) {
         if(ec) {
            ctx.stop();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_nodsig_socket);
         if(!success) {   //failure reading indicates that nodsig has shut down
            ctx.stop();
            return;
         }
         ...
         try {
            ...
            write_message_with_fds(_nodsig_socket, initalize_response_message());
         }
         catch(const fc::exception& e) {
            write_message_with_fds(_nodsig_socket, initalize_response_message{e.what()});
         }
         catch(...) {
            write_message_with_fds(_nodsig_socket, initalize_response_message{"Failed to create compile process"});
         }
   ...
   local::datagram_protocol::socket _nodsig_socket;
   wrapped_fd _trampoline_socket;

   std::list<compile_monitor_session> _compile_sessions;
};

void launch_compile_monitor(int nodsig_fd) {
...

   {
      ...
      boost::asio::local::datagram_protocol::socket nodsig_socket(ctx);
      nodsig_socket.assign(boost::asio::local::datagram_protocol(), nodsig_fd);
      wrapped_fd trampoline_socket(socks[0]);
      compile_monitor monitor(ctx, std::move(nodsig_socket), std::move(trampoline_socket));
      ctx.run();
