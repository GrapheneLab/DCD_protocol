 ...
 */
void apply_sig_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if( !creator.is_privileged() ) {
      EOS_ASSERT( name_str.find( "sig." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'sig.'" );
   }
...
void apply_sig_setcode(apply_context& context) {
...
void apply_sig_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_sig_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "sig." ) != 0, action_validate_exception,
               "Permission names that start with 'sig.' are reserved" );
...
void apply_sig_deleteauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_sig_linkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_sig_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_sig_canceldelay(apply_context& context) {
...
void apply_sig_setfee(apply_context& context) {
   ...
   asset base_rate_asset = context.control.get_txfee_manager().get_base_rate_asset(context.control);

   EOS_ASSERT( act.fee.get_symbol() == base_rate_asset.get_symbol(), account_query_exception, "Wrond base rate asset symbol");
