...
struct building_block {
   ...
   optional<producer_rate_info>	        _new_pending_rate_info;
...
struct assembled_block {
   ...
   optional<producer_rate_info>          _new_rate_info;
};
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( sig, sig, newaccount );
   SET_APP_HANDLER( sig, sig, setcode );
   SET_APP_HANDLER( sig, sig, setabi );
   SET_APP_HANDLER( sig, sig, setfee );
   SET_APP_HANDLER( sig, sig, updateauth );
   SET_APP_HANDLER( sig, sig, deleteauth );
   SET_APP_HANDLER( sig, sig, linkauth );
   SET_APP_HANDLER( sig, sig, unlinkauth );
   SET_APP_HANDLER( sig, sig, canceldelay );
   }
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
     ...
     // static int counter = 0;
      //counter++;
     // std::cout << " INCOMMING TRX COUNTER = " << counter << std::endl;
      ...
      try {
         ...
         try {
            ...
            const auto fee = txfee.get_required_fee(self, trn);
            EOS_ASSERT(fee == trn.fee, transaction_exception, "Wrong transaction fee");
                     ...
                     const optional<block_id_type>& producer_block_id )
   {
      ...
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete )
      {
         ...
         if( gpo.proposed_rate_block_num.valid()) {
            if (*gpo.proposed_rate_block_num <= pbhs.dpos_irreversible_blocknum) {
               if(pbhs.prev_pending_rate.rate_info.rate == 0) {
                  ...
                  pending->_block_stage.get<building_block>()._new_pending_rate_info = ri;
                  db.modify( gpo, [&]( auto& gp ) {
                     gp.proposed_rate_block_num = optional<block_num_type>();
   ...
   void commit_block( bool add_to_fork_db ) {
      txfee.get_new_rate_params(self);
...
void controller::preactivate_feature( const digest_type& feature_digest ) {
   ...
   // Even if they don't recognize the feature, the producer could change their nodsig code to treat it like an
...
const producer_rate_info&    controller::active_rate()const {
   ...
   if( my->pending->_block_stage.contains<completed_block>() )
      return my->pending->_block_stage.get<completed_block>()._block_state->active_rate;
...
const producer_rate_info& controller::pending_rate()const {
   ...
   if( my->pending->_block_stage.contains<completed_block>() )
      return my->pending->_block_stage.get<completed_block>()._block_state->pending_rate.rate_info;

   if( my->pending->_block_stage.contains<assembled_block>() ) {
      const auto& new_rate_cache = my->pending->_block_stage.get<assembled_block>()._new_rate_info;
      if( new_rate_cache ) {
...
optional<producer_rate_info> controller::proposed_rate()const {
   const auto& gpo = get_global_properties();
   if( !gpo.proposed_rate_block_num.valid() )
      return optional<producer_rate_info>();
