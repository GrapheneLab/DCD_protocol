...
function previous-install-prompt() {
  if [[ -d $EOSIO_INSTALL_DIR ]]; then
    echo "EOSIO has already been installed into ${EOSIO_INSTALL_DIR}... It's suggested that you sig_uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    ...
    echo "${COLOR_CYAN}EOSIO Stack Exchange:${COLOR_NC} https://sig.stackexchange.com"
}
