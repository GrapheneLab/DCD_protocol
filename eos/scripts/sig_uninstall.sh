...
function usage() {
  ...
  -i DIR      Directory where sig is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the sig data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
if [[ ! -d "${EOSIO_INSTALL_DIR}" ]]; then
   echo "[SIG installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${EOSIO_INSTALL_DIR}]"
else
   echo "[SIG installation found: ${EOSIO_INSTALL_DIR}]" && INSTALL_PATHS+=("${EOSIO_INSTALL_DIR}") # EOSIO_INSTALL_DIR set in .environment
   while true; do
      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/sig")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/sig")
            fi
            ...
            echo " - SIG Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled SIG Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing SIG Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/sig_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
