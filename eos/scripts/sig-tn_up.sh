#!/bin/bash
#
# sig-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# sig-tn_bounce.sh and sig-tn_roll.sh scripts.

connected="0"

rundir=programs/nodsig
prog=nodsig
...
if [ ! -e $rundir/$prog ]; then
    echo unable to locate binary for nodsig
    exit -1
fi
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/sig/node_$EOSIO_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/sig/node_$EOSIO_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
