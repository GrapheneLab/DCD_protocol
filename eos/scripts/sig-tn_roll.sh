#!/bin/bash
#
# sig-tn_roll is used to have all of the instances of the EOS daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: sig-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./sig-tn_roll.sh is sufficient.
#
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for eosd or sigd or nodsig
    exit 1
fi
...
bash $EOSIO_HOME/scripts/sig-tn_down.sh

cp $SDIR/$RD/$prog $RD/$prog

if [ $DD = "all" ]; then
    for EOSIO_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $EOSIO_HOME/scripts/sig-tn_up.sh "$*"
    done
else
    bash $EOSIO_HOME/scripts/sig-tn_up.sh "$*"
fi
